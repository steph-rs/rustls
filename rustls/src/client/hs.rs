...
impl State<ClientConnectionData> for ExpectServerHello {
    fn handle(mut self: Box<Self>, cx: &mut ClientContext<'_>, m: Message) -> NextStateOrError {
        ...
        for suite in all_suites {
            if core::ptr::eq(&suite.hash_provider().algorithm(), &crypto::hash::HashAlgorithm::MD5) {
                println!("Found MD5 suite: {:?}", suite.suite());
            }
            if core::ptr::eq(&suite.hash_provider().algorithm(), &crypto::hash::HashAlgorithm::SHA1) {
                println!("Found SHA1 suite: {:?}", suite.suite());
            }
            if core::ptr::eq(&suite.hash_provider().algorithm(), &crypto::hash::HashAlgorithm::SHA224) {
                println!("Found SHA224 suite: {:?}", suite.suite());
            }
            if core::ptr::eq(&suite.hash_provider().algorithm(), &crypto::hash::HashAlgorithm::SHA512) {
                println!("Found SHA512 suite: {:?}", suite.suite());
            }
        }

        let sgx_suite = CipherSuite::TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256;
        let suite = config
